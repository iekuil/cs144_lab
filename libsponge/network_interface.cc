...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    // 将frame插入todo_list
    if(insert_todolist(next_hop, frame)){
        // 构造相应的ARP请求并发送
        ARPMessage arp_req = make_ARPmessage(next_hop, std::nullopt);
...
std::optional<InternetDatagram> NetworkInterface::recv_frame(const EthernetFrame &frame) {
    ...
    sender_ip.from_ipv4_numeric(arp_message.sender_ip_address);
...
bool NetworkInterface::insert_todolist(const Address &ip, const EthernetFrame &frame){
